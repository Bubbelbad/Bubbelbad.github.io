...
let filmLibrary = new Project("FilmLibrary", "This application should let the user browse movies for inspiration. The user should be able to save the movies to different lists, like 'favorites' and 'watch later'. First I thought I'd use API's to get the data, but since I need to practice MySQL and database connection I am changing the program to use that instead. It might be that I change my goals when I know more, but this is the idea! The architecture of this app wasn't planned from scratch and thus the structure is a little bit messy. I learned a lot about UserControls though, and look forward to doing another project with the knowedge I accumulated in this app as a start.", ["MySQL", "WPF"]);

let battleShip = new Project("BattleShip", "My goal is to make an simplified single player version of this game, with only one game field with four hidden boats. The goal is to uncover all hidden boats in as few moves as possible. The player does not have it's own game field and won't get shot at by the CPU at this applications current state (but it might be developed later). \nThe player chooses coordinates A-J & 1-10 to aim at a spot. The player gets direct feedback if he hit a ship or not. After all boats are completely hit /uncovered, the game is won and the number of moves is saved to a highscore as CSV. The best scores are sorted through a priority queue to display the results in order.", ["Console App", "Game"]);

let theFarm = new Project("TheFarm", "Vi har fått i uppgift att utveckla ett program från ett redan existerande Klass-Diagram, samt en tillhörande relativt vag beskrivning. Vi delade upp klasserna emellan oss och hjälpte varandra koda den andres del över Discord.", ["Console App"]);
